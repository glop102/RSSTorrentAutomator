...
populate_next_file_info   | string | Given a folder, it will find some random file and then get the same information about it as get_file_info does. It also adds the the variable foundfile and sets it to either "true "or "false", as well as the variable relative_filepath which is the filepath relative to the folder being parsed.
